...
class ForeignApp extends Component {
    ...
    componentDidMount () {
        ...
        setTimeout(() => {
            let text = this.props.foreignSelectData.text || this.props.foreignSelectData.txt;
    ...
    setKeyWords = () => {
        ...
        let keyWordSearchObj = { Foreign: '1', txt: this.state.inputValue };
        this.setState({
            showSearchResult: showSearchResult,
            inputValue: this.state.inputValue,
            selectedVal: keyWordSearchObj
        });
        this.props.onClickItem({
            foreignSelectData: keyWordSearchObj
        });
    };
    ...
    creatSeeMoreUrl = () => {
        ...
        let urlSetting = process.env.ENV === 'prod' ? window.urlType : '//uactivity.liontravel.com';
        let seeMoreUrl = `${urlSetting}/search?Foreign=1&SearchKeyword=${encodeInputValue}&OriginSearchText=${encodeInputValue}`;
        return seeMoreUrl;
    };
    ...
    closeSearch = () => {
        ...
        this.setState({
            showSearchResult: false,
            foreignSelectData: { Foreign: 1, txt: this.state.inputValue }
        });
    };

    render () {
        const { foreign, foreignSelectData } = this.props;
        const {
            inputValue,
            innerValue,
            actData,
        ...
        const svgClassName = () => {
            ...
            if (Object.keys(foreignSelectData).length) {
                return '';
            } else {
                return 'd-no';
            }
                ...
                </div>
                {
                    ...
                    >
                        ...
                        <DtmRcfr
                            ...
                            dataResouce={
                                process.env.ENV === 'prod' ? window.dataResouceForeign : './json/activityabroad.json'
                            }
